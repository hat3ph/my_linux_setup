...
function send_notification() {
	volume=$($app get-volume @DEFAULT_AUDIO_SINK@ | cut -d ' ' -f 2 | awk '{printf "%2.0f%%\n", 100 * $1}')
...
up)
	...
	$app set-volume -l 1 @DEFAULT_AUDIO_SINK@ $audio_level%+
	send_notification
	;;
down)
...
mute)
	...
	$app set-mute @DEFAULT_AUDIO_SINK@ toggle
	# send audio muted notification if muted or send audio level notification if not muted
	if [[ -n $($app get-volume @DEFAULT_AUDIO_SINK@ | cut -d ' ' -f 3 | sed 's/^.//;s/.$//') ]]; then
...
mic)
	...
	$app set-mute @DEFAULT_AUDIO_SOURCE@ toggle
	# send notification if microphone is muted or un-muted
	if [[ -n $($app get-volume @DEFAULT_AUDIO_SOURCE@ | cut -d ' ' -f 3 | sed 's/^.//;s/.$//') ]]; then
